...
test('two same-direction losses become diagnostics and never suppress the next boundary signal',()=>{
  ...
  assert.equal(guarded.tradeQualified,true);
  assert.equal(guarded.qualified,true);
  assert.equal(guarded.features.repeatedDirectionGuard,false);
  assert.equal(guarded.features.repeatedDirectionVeto,false);
  assert.equal(guarded.features.repeatedDirectionRisk,true);
  assert.equal(guarded.features.lossLearningFrequencyPreserved,true);
  assert.match(guarded.vetoReasons.join('|'),/REPEATED_DIRECTION_STALE_EVIDENCE_RISK/);
});
...
test('frequency scoring keeps a rejected setup available without hiding its diagnostics',()=>{
  ...
  assert.deepEqual(tuned.vetoReasons,['TRANSITION_GATE','ENTRY_LANE_GATE']);
...
test('strongly contradicted direction can rerank without removing the signal',()=>{
...
test('all supported pair-timeframe tracks are eligible for evidence-based reranking while cadence stays preserved',()=>{
  ...
  const tuned=tuneFrequencyScore(signal);
  assert.equal(tuned.features.directionRerankEligible,true);
  assert.equal(tuned.direction,'SELL');
  assert.equal(tuned.tradeQualified,true);
  assert.equal(tuned.features.frequencyPreserved,true);
  assert.ok(tuned.confidence>=57);
