...
function freshEvidenceCredit(tags,f){
  const weights={DIRECTIONAL_EVIDENCE:.8,FRESH_PROGRESS:.9,CONSENSUS:.75,BREAKOUT_PROGRESS:.25,FVG_RESOLVED:.20,VSA_SUPPORT:.45,SEQUENCE_PRESSURE:.55,MOVE_QUALITY:.80};
...
export function tuneFrequencyScore(signal){
  ...
  return{
    ...
    vetoReasons:warnings,engine:`${signal?.engine||'FUSION_V2_11'}+DIRECTION_RERANK_V5+FREQUENCY_SCORE_V8+TRACK_CAL_V8`,
    features:{
      ...f,modelConfidence:Number(modelConfidence.toFixed(1)),modelQualified:signal?.qualified===true&&signal?.tradeQualified!==false,
      scoreOnlyWarnings:warnings,frequencyStrength:Number(strength.toFixed(4)),frequencyScoreVersion:'V8',frequencyPreserved:true,quotaPreserved:true,
