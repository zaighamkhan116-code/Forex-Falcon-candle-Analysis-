...
function conflictState(signal,rank,quality){
  ...
  if(f.emaCompression===true)flags.push('EMA_COMPRESSION');
  if(f.bbContraction===true)flags.push('BB_CONTRACTION');
...
function freshEvidence(f,direction,alignedEvidence,quality){
  const tags=[],sign=direction==='BUY'?1:-1,progress=Number(f.progressScore||0)*sign,consensusDirection=upper(f.groupConsensusDirection),dominance=Number(f.groupDominance||0),fvg=upper(f.activeFvgState),progressConfirmed=f.failureToProgress!==true&&progress>=2.5&&quality>=.55;
  ...
  // Gap resolution is context, not confirmation, until price actually progresses.
  if(['REJECTED','ACCEPTED_THROUGH','FULLY_MITIGATED'].includes(fvg)&&progressConfirmed)tags.push('FVG_RESOLVED_CONFIRMED');
...
function freshEvidenceCredit(tags,f){
  const weights={DIRECTIONAL_EVIDENCE:.8,FRESH_PROGRESS:.9,CONSENSUS:.75,BREAKOUT_PROGRESS:.25,FVG_RESOLVED_CONFIRMED:.20,VSA_SUPPORT:.45,SEQUENCE_PRESSURE:.55,MOVE_QUALITY:.80};
  let credit=tags.reduce((sum,t)=>sum+(weights[t]||0),0);
  if(f.breakoutMaturityGate===true&&tags.includes('BREAKOUT_PROGRESS'))credit+=.8;
  if(f.continuationResetGate===true&&tags.includes('FRESH_PROGRESS'))credit+=.55;
  if(f.progressFailureGate===true&&tags.includes('FRESH_PROGRESS'))credit+=.55;
  if(f.maBbAlignment===true||f.maBbAligned===true)credit+=.65;
  if(f.tickAcceleration===true)credit+=.65;
  return Math.min(4.5,credit);
}
...
function structuralRisk(f,direction,conflicts){
  const sign=direction==='BUY'?1:-1,progress=Number(f.progressScore||0)*sign,
    ...
    compression=f.emaCompression===true||f.bbContraction===true;
  let score=0;
  if(late)score+=1.2;if(adverse)score+=1.5;if(weakProgress)score+=1.7;if(transition)score+=1.1;if(mtf)score+=1;if(unresolved)score+=.8;if(extended)score+=.7;if(compression)score+=.9;
  const compound=(adverse&&weakProgress?1.4:0)+(late&&weakProgress?1.1:0)+(mtf&&transition?.8:0)+(extended&&unresolved?.6:0)+(compression&&weakProgress?.9:0)+(compression&&mtf?.6:0);
  return{score:Number((score+compound).toFixed(2)),late,extended,transition,weakProgress,adverse,mtf,unresolved,compression};
}
...
export function tuneFrequencyScore(signal){
  const f=signal?.features||{},modelConfidence=Number(signal?.confidence||50),rank=rerankDirection(signal),
    ...
    staleCompound=risk.weakProgress&&[risk.adverse,risk.late,risk.unresolved,risk.transition,risk.mtf,risk.compression].filter(Boolean).length>=2,
    effectiveEvidenceSupport=staleCompound?Math.min(.5,evidenceSupport):evidenceSupport,
    effectiveConsensusSupport=staleCompound?Math.min(.5,consensusSupport):consensusSupport,
    strength=.35*effectiveEvidenceSupport+.20*quality+.20*effectiveConsensusSupport+.10*tickScore+.15*directionEdge,
    rerankConviction=rank.reranked?clamp(rank.margin/8,0,1):0,
    compoundConflictExtra=conflicts.flags.length>=4?(conflicts.flags.length-3)*.65:0,
    conflictPenalty=Math.min(9,conflicts.flags.length*1.05+(alignedEvidence<0?Math.abs(alignedEvidence)*2:0)+compoundConflictExtra),
    warningPenalty=Math.min(4,warnings.length*.6),freshCredit=freshEvidenceCredit(fresh,f),structuralPenalty=Math.min(10,risk.score),
  ...
  return{
    ...
    vetoReasons:warnings,engine:`${signal?.engine||'FUSION_V2_11'}+DIRECTION_RERANK_V5+FREQUENCY_SCORE_V8_1+TRACK_CAL_V8`,
    features:{
      ...f,modelConfidence:Number(modelConfidence.toFixed(1)),modelQualified:signal?.qualified===true&&signal?.tradeQualified!==false,
      scoreOnlyWarnings:warnings,frequencyStrength:Number(strength.toFixed(4)),frequencyScoreVersion:'V8_1_STALE_COMPOUND_GUARD',frequencyPreserved:true,quotaPreserved:true,
      ...
      frequencyConflicts:conflicts.flags,frequencyConflictPenalty:Number(conflictPenalty.toFixed(2)),frequencyCompoundConflictExtra:Number(compoundConflictExtra.toFixed(2)),frequencyStructuralRisk:Number(risk.score.toFixed(2)),
      ...
      frequencyStaleCompoundOverride:staleCompound,frequencyEvidenceSupportApplied:Number(effectiveEvidenceSupport.toFixed(4)),frequencyConsensusSupportApplied:Number(effectiveConsensusSupport.toFixed(4)),
      directionAlignedEvidence:Number(alignedEvidence.toFixed(4)),directionRerankVersion:'V5',directionRerankEligible:rank.eligible,directionReranked:rank.reranked,
      ...
      trackCalibrationVersion:trackCalibration.version,trackCalibrationTrack:trackCalibration.track,trackCalibrationSession:trackCalibration.session,trackCalibrationTimeBucket:trackCalibration.timeBucket,
      trackCalibrationAdjustment:trackCalibration.confidenceAdjustment,trackCalibrationDirectionBias:trackCalibration.directionBias,trackCalibrationTags:trackCalibration.tags,
      trackReactionQuality:trackCalibration.reactionQuality,trackMatureDisplacement:trackCalibration.matureDisplacement,trackCleanTrend:trackCalibration.cleanTrend,
      trackFailedProgress:trackCalibration.failedProgress,trackCompoundConflict:trackCalibration.compoundConflict,trackWeakMove:trackCalibration.weakMove,trackCompressed:trackCalibration.compressed,trackLate:trackCalibration.late,
