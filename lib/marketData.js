...
async function series(pair,interval,outputsize){
 ...
 creditsUsed+=1;return normalize(j.values,interval);
}
export async function getPredictionBundle(pair){
 ...
 const [m1,m5,m15,h1]=await Promise.all([series(pair,'1min',320),series(pair,'5min',220),series(pair,'15min',180),series(pair,'1h',120)]);
