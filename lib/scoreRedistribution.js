...
export function assessScoreRedistribution(features={},risk={},direction='',horizon=1){
  ...
  // Repeated forward losses clustered around these stale-continuation combinations.
  ...
  // Compensating credits preserve frequency when genuinely fresh evidence appears.
  ...
  penalty=clamp(penalty*scale,0,7.5);
  bonus=clamp(bonus*scale,0,5.5);

  // Fresh evidence can offset stale penalties, but cannot turn a severely stale setup into a huge score boost.
  const net=clamp(bonus-penalty,-6,4);
  return{
    adjustment:Number(net.toFixed(1)),
    penalty:Number(penalty.toFixed(1)),
    bonus:Number(bonus.toFixed(1)),
    penaltyTags:penaltyTags.join('|')||'NONE',
    bonusTags:bonusTags.join('|')||'NONE',
    stalePressure:penalty>=3,
    freshPressure:bonus>=2,
    freshBreakout,freshSweep,fvgResolved,renewedProgress,renewedConsensus,cleanMtf,maBbAligned
  };
}
