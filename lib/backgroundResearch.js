...
import {getForexTickSnapshot,applyForexTicks} from './forexTicks.js';
...
async function predict(pair){const s=S(pair);try{await settle(pair);const bundle=await getStagedSnapshot(pair);let r=analyze(bundle,1,pair);r=await enrich(r,pair);const now=Date.now(),boundary=Math.ceil((now+1000)/60000)*60000,wait=Math.max(0,boundary-Date.now()+25);if(wait)await new Promise(x=>setTimeout(x,wait));const entry=await getSettlementPrice(pair);s.pending.push({id:`${pair}-${boundary}`,pair,symbol:marketSymbol(pair),horizon:1,predictedAt:now,signalBoundary:boundary,entry:entry.price,entryLockedAt:Date.now(),expiry:boundary+60000,direction:r.direction,confidence:Number(clamp(Number(r.confidence)||50,50,100).toFixed(1)),regime:r.regime,engine:r.engine||'TECHNICAL',features:r.features||{},result:'PENDING',quotexRule:true});s.lastRunAt=Date.now();s.lastError=null;}catch(e){s.lastError=e.message;}}
function schedule(pair){const s=S(pair);s.running=true;const now=Date.now(),next=Math.ceil(now/180000)*180000-30000,delay=Math.max(1000,next-now);const t=setTimeout(async()=>{if(!s.running)return;await predict(pair);schedule(pair);},delay);timers.set(pair,t);}
