...
async function analyzeFresh(s,boundary){
  s.scans++;
  try{
    const rawBundle=await getStagedSnapshot(s.pair),bundle=timeframeBundle(rawBundle,s.horizon),raw=analyze(bundle,s.horizon,s.pair),enriched=await enrich({...raw,horizon:s.horizon,features:{...(raw.features||{}),horizon:s.horizon,analysisTimeframe:bundle.analysisTimeframe||`${s.horizon}M`}},s.pair,s.horizon);
