...
function applyForwardLossStreakCalibration(result,pair,horizon){
  ...
  if(!trigger||!['BUY','SELL'].includes(dir))return{result:{...result,features:{...f,lossStreakCalibrationVersion:'V3_FORWARD_VALIDATION',lossStreakRerankApplied:false}},calibration:null};
  ...
  const calibration={version:'V3_FORWARD_VALIDATION',status:'FORWARD_VALIDATION',trigger,evidence,originalDirection:dir,rerankedDirection:newDirection,frequencyImpact:'NONE',globalThresholdChanged:false};
  return{result:{...result,direction:newDirection,evidenceScore:newEvidence,qualified:true,tradeQualified:true,minimumConfidence:BASE_THRESHOLD,features:{...f,lossStreakCalibrationVersion:'V3_FORWARD_VALIDATION',lossStreakRerankApplied:true,lossStreakRerankTrigger:trigger,lossStreakRerankEvidence:evidence,lossStreakOriginalDirection:dir,lossStreakRerankedDirection:newDirection,lossStreakFrequencyImpact:'NONE'}},calibration};
}
...
export function applyLossContext(result,pair,horizon){
  ...
  if(!ctx||ctx.remaining<=0)return{result:{...baseResult,qualified:true,tradeQualified:true,minimumConfidence:BASE_THRESHOLD,features:{...f,lossLearningFrequencyPreserved:true,lossLearningVersion:'V3_FORWARD_VALIDATION'}},context:calibrated.calibration,minimumConfidence:BASE_THRESHOLD};
  ...
  return{result:{...baseResult,qualified:true,tradeQualified:true,minimumConfidence:BASE_THRESHOLD,vetoReasons:scoreOnlyWarnings,adaptiveContext:context,features:{...f,repeatedDirectionGuard:false,repeatedDirectionVeto:false,repeatedDirectionRisk,repeatedDirectionFreshEvidence:freshEvidence,repeatedDirectionGuardVersion:'V3_FORWARD_VALIDATION',lossLearningFrequencyPreserved:true,lossLearningDiagnosticPenalty:Number(diagnosticPenalty.toFixed(2)),lossLearningDiagnosticCredit:Number(diagnosticCredit.toFixed(2))}},context,minimumConfidence:BASE_THRESHOLD}
}
