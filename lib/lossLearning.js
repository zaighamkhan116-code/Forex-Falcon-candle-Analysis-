...
function contextRouteAssessment(result,pair,horizon,direction){
  const f=result?.features||{},p=String(pair||'').toUpperCase(),h=Number(horizon),dir=String(direction||'').toUpperCase();
  if(!['BUY','SELL'].includes(dir))return{version:'V3_7_CONTEXT_ROUTER',marketState:'UNKNOWN',active:false,rerankEligible:false,opposingFamilies:[],opposingFamilyCount:0};
  const regime=String(result?.regime||'').toUpperCase(),sr=String(f.sr||f.dynamicZoneSide||'').toUpperCase(),liq=String(f.liquidity||'').toUpperCase(),breakout=String(f.breakout||'').toUpperCase(),structure=String(f.structure||'').toUpperCase();
  const choppy=regime==='CHOPPY'||f.microRangeChop===true||f.transitionCompression===true||String(f.bbState||'').toUpperCase().includes('SQUEEZE');
  const atUpper=sr.includes('RESISTANCE')||liq==='BUY-SIDE SWEEP',atLower=sr.includes('SUPPORT')||liq==='SELL-SIDE SWEEP';
  const adverseLocation=(dir==='BUY'&&atUpper)||(dir==='SELL'&&atLower);
  const failedProgress=f.failureToProgress===true||f.progressFailureRisk===true||Number(f.moveQualityScore||6)<5.5;
  const breakoutPresent=f.breakoutAccepted===true||f.extendedMove===true||breakout.includes('BREAK');
  const matureRisk=f.breakoutMaturityGate===false||f.breakoutMaturityRiskActive===true||f.fragileAcceptedBreakout===true||(breakoutPresent&&failedProgress);
  const stale=f.continuationFreshEvidence===false||f.continuationResetRequired===true||f.continuationResetGate===false;
  const trendAligned=(dir==='BUY'&&(Number(f.trend||0)>.08||structure.includes('BULL')))||(dir==='SELL'&&(Number(f.trend||0)<-.08||structure.includes('BEAR')));
  const failedBreakout=(dir==='BUY'&&breakout.includes('FAILED BULL'))||(dir==='SELL'&&breakout.includes('FAILED BEAR'))||(breakoutPresent&&failedProgress&&adverseLocation);
  let marketState='TRANSITION';
  if(failedBreakout)marketState='FAILED_BREAKOUT';
  else if(choppy&&(atUpper||atLower))marketState='RANGE_EXTREME';
  else if(choppy)marketState='RANGE_MIDDLE';
  else if(breakoutPresent&&!matureRisk&&f.continuationFreshEvidence===true)marketState='BREAKOUT';
  else if(trendAligned&&stale)marketState='TREND_RETRACEMENT';
  else if(trendAligned)marketState='TREND_CONTINUATION';
  const opposingFamilies=directionalOpposition(f,dir),originalScore=Number(f.originalDirectionScore),oppositeScore=Number(f.oppositeDirectionScore),scoreNotSupportingOriginal=Number.isFinite(originalScore)&&Number.isFinite(oppositeScore)&&oppositeScore>=originalScore-.25;
  const freshContinuation=f.continuationFreshEvidence===true&&f.breakoutAccepted===true&&f.failureToProgress!==true&&Number(f.moveQualityScore||0)>=6.5&&Number(f.confirmationV2Opposed||0)<=1;
  const evidenceCount=opposingFamilies.length+(adverseLocation?1:0)+(failedProgress?1:0)+(stale?1:0);
  const rerankEligible=p==='EURUSD'&&h===2&&choppy&&matureRisk&&!freshContinuation&&evidenceCount>=4&&(opposingFamilies.length>=2||adverseLocation)&&scoreNotSupportingOriginal;
  return{version:'V3_7_CONTEXT_ROUTER',status:'FORWARD_VALIDATION',marketState,active:p==='EURUSD'&&h===2,rerankEligible,choppy,matureRisk,failedProgress,stale,adverseLocation,opposingFamilies,opposingFamilyCount:opposingFamilies.length,evidenceCount,scoreNotSupportingOriginal,freshContinuation,frequencyImpact:'NONE',globalThresholdChanged:false,shadowInfluence:'NONE'};
}
...
export function applyForwardTrackCalibration(result,pair,horizon){
  ...
  const contextRoute=contextRouteAssessment(result,p,h,dir);
  ...
  else if(p==='EURUSD'&&h===2&&contextRoute.rerankEligible){trigger='EURUSD_2M_CONTEXT_ROUTER_RERANK';evidence=`B63_BREAKOUT_MATURITY_0W_3L_${contextRoute.marketState}_${contextRoute.opposingFamilies.join('+')}`}
  ...
  const contextTelemetry={contextRouterVersion:contextRoute.version,contextRouterStatus:contextRoute.status||'FORWARD_VALIDATION',contextMarketState:contextRoute.marketState,contextRouterActive:contextRoute.active,contextRouterRerankEligible:contextRoute.rerankEligible,contextRouterChoppy:contextRoute.choppy||false,contextRouterMatureRisk:contextRoute.matureRisk||false,contextRouterFailedProgress:contextRoute.failedProgress||false,contextRouterStale:contextRoute.stale||false,contextRouterAdverseLocation:contextRoute.adverseLocation||false,contextRouterOpposingFamilies:contextRoute.opposingFamilies||[],contextRouterOpposingFamilyCount:contextRoute.opposingFamilyCount||0,contextRouterEvidenceCount:contextRoute.evidenceCount||0,contextRouterFrequencyImpact:'NONE',contextRouterShadowInfluence:'NONE'};
  const candidateTelemetry={lossStreakCandidateVersion:'V3_5_TARGETED_COMPOUND_SHADOW',lossStreakCandidateActive:Boolean(candidateTrigger),lossStreakCandidateTrigger:candidateTrigger,lossStreakCandidateEvidence:candidateEvidence,lossStreakCandidateDirection:candidateTrigger&&['BUY','SELL'].includes(dir)?opposite(dir):null,lossStreakCandidateFrequencyImpact:'NONE',lossStreakOpposedFamilies:opposedFamilies,lossStreakOpposedFamilyCount:opposedFamilyCount,...transitionTelemetry,...contextTelemetry};
  if(!trigger||!['BUY','SELL'].includes(dir))return{result:{...result,features:{...f,...candidateTelemetry,lossStreakCalibrationVersion:'V3_7_CONTEXT_ROUTER_FORWARD_VALIDATION',lossStreakRerankApplied:false}},calibration:null};
  const newDirection=opposite(dir),newEvidence=Number.isFinite(Number(result?.evidenceScore))?-Number(result.evidenceScore):result?.evidenceScore,generic=trigger==='CROSS_TRACK_TRANSITION_DIRECTION_RERANK',contextual=trigger==='EURUSD_2M_CONTEXT_ROUTER_RERANK';
  const contextConfidence=BASE_THRESHOLD+Math.min(9,(contextRoute.opposingFamilyCount||0)*2+(contextRoute.adverseLocation?1:0)+(contextRoute.failedProgress?1:0));
  const confidence=contextual?clamp(contextConfidence,BASE_THRESHOLD,66):generic?clamp(Number(result?.confidence??BASE_THRESHOLD),BASE_THRESHOLD,66):result?.confidence;
  const version=contextual?'V3_7_EURUSD_2M_CONTEXT_ROUTER_RERANK':generic?'V3_6_CROSS_TRACK_TRANSITION_RERANK':'V3_5_TARGETED_COMPOUND_RERANK';
  const calibration={version,status:'FORWARD_VALIDATION',trigger,evidence,originalDirection:dir,rerankedDirection:newDirection,frequencyImpact:'NONE',globalThresholdChanged:false,shadowInfluence:'NONE',marketState:contextRoute.marketState,confidenceRecalibrated:contextual};
  return{result:{...result,direction:newDirection,confidence,evidenceScore:newEvidence,qualified:true,tradeQualified:true,minimumConfidence:BASE_THRESHOLD,features:{...f,...candidateTelemetry,lossStreakCalibrationVersion:version,lossStreakRerankApplied:true,lossStreakRerankTrigger:trigger,lossStreakRerankEvidence:evidence,lossStreakOriginalDirection:dir,lossStreakRerankedDirection:newDirection,lossStreakFrequencyImpact:'NONE',lossStreakConflictCount:breakoutProgressConflictCount,lossStreakRuntimeConflictCount:runtimeConflictCount,transitionDirectionOriginalDirection:generic?dir:null,transitionDirectionRerankedDirection:generic?newDirection:null,contextRouterOriginalDirection:contextual?dir:null,contextRouterFinalDirection:contextual?newDirection:null,contextRouterOriginalConfidence:contextual?Number(result?.confidence??BASE_THRESHOLD):null,contextRouterFinalConfidence:contextual?confidence:null}},calibration};
}
...
export function applyLossContext(result,pair,horizon){
  ...
  if(!ctx||ctx.remaining<=0)return{result:{...baseResult,qualified:true,tradeQualified:true,minimumConfidence:BASE_THRESHOLD,features:{...f,lossLearningFrequencyPreserved:true,lossLearningVersion:'V3_7_CONTEXT_ROUTER_FORWARD_VALIDATION'}},context:calibrated.calibration,minimumConfidence:BASE_THRESHOLD};
