...
function applyForwardLossStreakCalibration(result,pair,horizon){
  ...
  const audusd1ProgressFailureConflict=f.progressFailureRisk===true&&staleContinuation&&runtimeConflictCount>=1;
  const transitionGateFailed=f.transitionGate===false||f.transitionRiskActive===true;
  const scoreNotSupportingOriginal=Number.isFinite(originalScore)&&Number.isFinite(oppositeScore)?oppositeScore>=originalScore-.25:true;
...
export function applyLossContext(result,pair,horizon){
  ...
  return{result:{...baseResult,qualified:true,tradeQualified:true,minimumConfidence:BASE_THRESHOLD,vetoReasons:scoreOnlyWarnings,adaptiveContext:context,features:{...f,repeatedDirectionGuard:false,repeatedDirectionVeto:false,repeatedDirectionRisk,repeatedDirectionFreshEvidence:freshEvidence,repeatedDirectionGuardVersion:'V3_3_FORWARD_VALIDATION',lossLearningFrequencyPreserved:true,lossLearningDiagnosticPenalty:Number(diagnosticPenalty.toFixed(2)),lossLearningDiagnosticCredit:Number(diagnosticCredit.toFixed(2))}},context,minimumConfidence:BASE_THRESHOLD}
}
