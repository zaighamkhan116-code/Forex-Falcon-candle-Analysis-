...
export function attachShadowPrediction(signal,options={}){
  ...
  const run=async()=>{let candles=options.candles||[];if(options.candlesPromise){try{candles=await options.candlesPromise}catch(e){return{status:'UNAVAILABLE',model:'RF+EXTRATREES+HISTGB_SHADOW_V1',reason:`CANDLE_HISTORY_UNAVAILABLE: ${e?.message||e}`,influencedLiveSignal:false}}}return requestShadowPrediction(signal,{...options,candles})};
  run().then(x=>{signal.shadowModel=x}).catch(e=>{signal.shadowModel={status:'UNAVAILABLE',model:'RF+EXTRATREES+HISTGB_SHADOW_V1',reason:e?.message||String(e),influencedLiveSignal:false}});
  return signal;
}
