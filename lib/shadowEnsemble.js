...
export async function requestShadowPrediction(signal,{url=process.env.SHADOW_MODEL_URL||DEFAULT_URL,timeoutMs=Number(process.env.SHADOW_MODEL_TIMEOUT_MS||DEFAULT_TIMEOUT_MS),fetchImpl=globalThis.fetch,candles=[]}={}){
  ...
  try{
    ...
    if(!res.ok){const reason=await readError(res);if(res.status===422){return{status:'UNSUPPORTED',model:'RF+EXTRATREES+HISTGB_SHADOW_V1',requestedAt,httpStatus:res.status,reason,influencedLiveSignal:false}}shadowHealth={...shadowHealth,status:'UNAVAILABLE',lastCheckedAt:Date.now(),lastError:reason};return{status:'UNAVAILABLE',model:'RF+EXTRATREES+HISTGB_SHADOW_V1',requestedAt,httpStatus:res.status,reason,influencedLiveSignal:false}}
