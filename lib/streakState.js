...
function signature(signal={}){
  ...
  if(f.highConfidenceContradictionRisk===true)tags.push('HIGH_CONFIDENCE_CONTRADICTION');
...
export function applyStreakState(signal={}){
  ...
  // Cross-track only: repeated evidence is evaluated inside the exact pair×horizon state.
  // Existing high-confidence contradiction telemetry is now eligible for the same repeat-loss learner;
  // it never acts on a single event and never changes qualification/cadence or the 57 floor.
  if(st.lossRun>=2){
    for(const t of tags){
  ...
  return{...signal,confidence:Number(clamp(Number(signal.confidence||57)+adjustment,57,82).toFixed(1)),features:{...f,streakStateVersion:'V6.4_REPEAT_CONTRADICTION_STATE',streakLossRun:st.lossRun,streakWinRun:st.winRun,streakAdjustment:Number(adjustment.toFixed(2)),streakStateTags:tags,streakStateReasons:[...new Set(reasons)],pairSpecificForwardCalibration:false,highConfidenceContradictionRepeatLearning:true}};
}
