...
export function applyStreakState(signal={}){
  ...
  // but no hard-coded pair or timeframe receives an extra calibration.
  if(st.lossRun>=2){
    for(const t of tags){
  ...
  return{...signal,confidence:Number(clamp(Number(signal.confidence||57)+adjustment,57,82).toFixed(1)),features:{...f,streakStateVersion:'V6.3_CROSS_TRACK_REPEAT_STATE',streakLossRun:st.lossRun,streakWinRun:st.winRun,streakAdjustment:Number(adjustment.toFixed(2)),streakStateTags:tags,streakStateReasons:[...new Set(reasons)],pairSpecificForwardCalibration:false}};
}
