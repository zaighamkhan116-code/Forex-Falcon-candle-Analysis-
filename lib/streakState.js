...
export function applyStreakState(signal={}){
  ...
  // EURUSD 1M B42 forward evidence: CHOPPY + late/reset + low efficiency without fresh continuation
  // repeatedly lost. Keep cadence/floor unchanged; only compress confidence inside this exact regime.
  const eurusd1mStaleChop=String(signal.pair||'').toUpperCase()==='EURUSD'&&Number(signal.horizon||1)===1&&String(signal.regime||'').toUpperCase()==='CHOPPY'&&f.lateCounterTrendRisk===true&&f.continuationResetRequired===true&&f.continuationFreshEvidence!==true&&Number(f.efficiency??1)<.22;
  if(eurusd1mStaleChop&&st.lossRun>=2){adjustment-=1.35;reasons.push('EURUSD_1M_STALE_CHOP_B42_FORWARD_VALIDATION')}
  ...
  return{...signal,confidence:Number(clamp(Number(signal.confidence||57)+adjustment,57,82).toFixed(1)),features:{...f,streakStateVersion:'V6.2_EURUSD_1M_STALE_CHOP_FV',streakLossRun:st.lossRun,streakWinRun:st.winRun,streakAdjustment:Number(adjustment.toFixed(2)),streakStateTags:tags,streakStateReasons:[...new Set(reasons)],eurusd1mStaleChopForwardValidation:eurusd1mStaleChop}};
}
