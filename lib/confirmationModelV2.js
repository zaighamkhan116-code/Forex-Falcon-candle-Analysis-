...
export function confirmationModelV2({features={},direction,horizon=1}){
  ...
  if(lower>=.30&&lower>upper*1.25&&bullLoc){
    ...
    // Compatibility path: the live engine already classifies wick rejection patterns.
    // Until raw ratios are present in every collector payload, combine that causal
    // pattern with location so the 1-10 family is active rather than falsely neutral.
  ...
  return{directionalScore:Number(score.toFixed(4)),influence,contribution:Number((score*influence).toFixed(4)),alignedFamilies:aligned,opposedFamilies:opposed,familyCount:families.length,breadth:Number(breadth.toFixed(3)),conflict:Number(conflict.toFixed(3)),regimeScale,candleLocationScore:anatomyPoints,candleLocationBias:anatomyLabel,families:families.map(f=>({name:f.name,score:Number(f.raw.toFixed(3)),weight:f.weight}))};
}
