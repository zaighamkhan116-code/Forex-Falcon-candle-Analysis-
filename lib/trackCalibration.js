...
function applyPairPolicy(pair,horizon,m,ctx){
  ...
  if(pair==='AUDUSD'){
    ...
    if(horizon===2){
      ...
      if((m.compoundConflict>=3&&(m.failed||m.weakReaction))||(m.weakReaction&&m.mtfOppose>=2))add(-2.4,-1.0,'AUDUSD_2M_FORWARD_VALIDATION_CONFLICT_V9');
...
export function assessTrackCalibration(signal,direction=upper(signal?.direction||'BUY')){
  ...
  return{
    version:'V9_FORWARD_VALIDATION_TRACK_CONFLICT_CALIBRATION',pair,horizon,track:`${pair}:${horizon}:${ctx.session}:${ctx.bucket}`,direction,
