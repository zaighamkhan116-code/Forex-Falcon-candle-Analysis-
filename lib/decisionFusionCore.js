...
export function fuseDecision(result,bundle,pair,horizon=1,minimumConfidence=57){
  ...
  const rawPositive=Math.max(0,qualityAdjustment)+Math.max(0,tickAdjustment)+Math.max(0,consensusAdjustment),positiveFamilies=[qualityAdjustment>0,tickAdjustment>0,consensusAdjustment>0].filter(Boolean).length,independentPositive=Math.min(rawPositive,positiveFamilies<=1?3.2:positiveFamilies===2?5.4:7);
  const cappedNegative=Math.min(7,correlatedNegative);
  const offset=Math.min(cappedNegative*.45,independentPositive*.5);
  const totalAdjustment=clamp(-phasePenalty-cappedNegative+offset+independentPositive, -12, 10);
  ...
  const oneMinuteChopGuard=!(Number(horizon)===1&&phase.chop&&!phase.breakoutAccepted)||(mq.score>=6&&cons.direction===direction&&cons.votes>=3&&cons.opposing===0&&cons.dominance>=.45&&progressAligned>=3);
  const qualified=confidence>=minimumConfidence&&qualityGate&&oneMinuteChopGuard;
  features={...features,moveQualityScore:mq.score,moveQualityState:mq.label,progressScore:progress.score,progressState:progress.label,failureToProgress:progress.failure,dynamicZoneSide:zone.side,dynamicZoneStrength:zone.strength,dynamicZoneRejections:zone.rejections,dynamicZoneLevel:zone.level,tickReliabilityScore:tick.score,tickReliabilityState:tick.label,groupConsensusDirection:cons.direction,groupConsensusVotes:cons.votes,groupOpposingVotes:cons.opposing,groupBullStrength:cons.bull,groupBearStrength:cons.bear,groupDominance:cons.dominance,groupScores:cons.groups,qualityAdjustment:Number(qualityAdjustment.toFixed(1)),locationAdjustment:Number(locationAdjustment.toFixed(1)),tickAdjustment:Number(tickAdjustment.toFixed(1)),consensusAdjustment:Number(consensusAdjustment.toFixed(1)),rawPositiveAdjustment:Number(rawPositive.toFixed(1)),positiveEvidenceFamilies:positiveFamilies,deduplicatedPositiveAdjustment:Number(independentPositive.toFixed(1)),cappedNegativeAdjustment:Number(cappedNegative.toFixed(1)),positiveOffset:Number(offset.toFixed(1)),totalFusionAdjustment:Number(totalAdjustment.toFixed(1)),preFusionConfidence:Number(conf0.toFixed(1)),postFusionConfidence:Number(confidence.toFixed(1)),fusionQualityGate:qualityGate,qualityGateDiagnosticOnly:false,oneMinuteChopGuard,oneMinuteChopGuardActive:Number(horizon)===1&&phase.chop&&!phase.breakoutAccepted,sharedDecisionPath:true,confidenceCompressionFix:true,correlatedEvidenceDedup:true};
  return{...result,direction,confidence:Number(confidence.toFixed(1)),qualified,minimumConfidence,engine:`${result?.engine||'TECHNICAL'}+FUSION_V2_4`,features};
}
