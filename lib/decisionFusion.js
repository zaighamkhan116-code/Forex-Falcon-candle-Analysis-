...
export function fuseDecision(result,bundle,pair,horizon=1,minimumConfidence=62){
  const c=bundle?.m1||[],baseFeatures={...(result?.features||{})},dir0=String(result?.direction||'BUY').toUpperCase(),conf0=Number(result?.confidence||50),phase=phaseState(pair,horizon,c,dir0),phasePenalty=phase.exhaustionPenalty+phase.zonePenalty+phase.chopPenalty;
  let direction=dir0,confidence=clamp(conf0-phasePenalty,50,100);
  let features={...baseFeatures,nextCandlePhase:phase.phase,nextCandlePhaseScore:phase.phaseScore,prePhaseConfidence:Number(conf0.toFixed(1)),phaseAdjustedConfidence:Number(confidence.toFixed(1)),exhaustionPenalty:phase.exhaustionPenalty,zoneRejectionPenalty:phase.zonePenalty,microRangePenalty:phase.chopPenalty,breakoutAccepted:phase.breakoutAccepted,repeatedUpperZone:phase.repeatedUpper,repeatedLowerZone:phase.repeatedLower,microRangeChop:phase.chop,bullExtended:phase.bullExtended,bearExtended:phase.bearExtended,lastBodyRatio:phase.lastBodyRatio,lastUpperWickRatio:phase.lastUpperWickRatio,lastLowerWickRatio:phase.lastLowerWickRatio};
  ...
  let dirSign=direction==='BUY'?1:-1,progressAligned=progress.score*dirSign;if(progressAligned>=6)confidence+=3;else if(progressAligned<=-3)confidence-=5;if(mq.score>=7)confidence+=2;else if(mq.score<5)confidence-=4;if(zone.side==='RESISTANCE'&&direction==='BUY')confidence-=Math.min(6,zone.strength*.65);if(zone.side==='SUPPORT'&&direction==='SELL')confidence-=Math.min(6,zone.strength*.65);if(tick.score>0&&tick.score<5)confidence-=1.5;if(tick.score>=7&&features.forexTickAgreement===true)confidence+=2;
  if(cons.direction!==direction&&cons.votes>=2&&cons.dominance>=.35&&confidence<70){direction=cons.direction;confidence=Math.max(minimumConfidence,confidence-2+Math.min(8,cons.dominance*10));dirSign=direction==='BUY'?1:-1;progressAligned=progress.score*dirSign}else if(cons.direction===direction&&cons.votes>=3&&cons.opposing===0)confidence+=Math.min(6,2+cons.dominance*5);
  confidence=clamp(confidence,50,85);const qualityGate=mq.score>=4.5||(zone.strength>=7&&((zone.side==='SUPPORT'&&direction==='BUY')||(zone.side==='RESISTANCE'&&direction==='SELL'))),qualified=confidence>=minimumConfidence&&qualityGate;
  features={...features,moveQualityScore:mq.score,moveQualityState:mq.label,progressScore:progress.score,progressState:progress.label,failureToProgress:progress.failure,dynamicZoneSide:zone.side,dynamicZoneStrength:zone.strength,dynamicZoneRejections:zone.rejections,dynamicZoneLevel:zone.level,tickReliabilityScore:tick.score,tickReliabilityState:tick.label,groupConsensusDirection:cons.direction,groupConsensusVotes:cons.votes,groupOpposingVotes:cons.opposing,groupBullStrength:cons.bull,groupBearStrength:cons.bear,groupDominance:cons.dominance,groupScores:cons.groups,preFusionConfidence:Number(conf0.toFixed(1)),postFusionConfidence:Number(confidence.toFixed(1)),fusionQualityGate:qualityGate,sharedDecisionPath:true};
  return{...result,direction,confidence:Number(confidence.toFixed(1)),qualified,minimumConfidence,engine:`${result?.engine||'TECHNICAL'}+FUSION_V2`,features};
}
